...
def parse_args():
    ...
    parser.add_argument(
        "-p", "--honeypot", help="Type of honeypot for this sensor", required=True
    )
    parser.add_argument(
        "-d", "--deploy-key", help="Deploy Key for registration", required=True
    )
    parser.add_argument(
        "-n", "--hostname", help="Hostname of honeypot", default=socket.gethostname()
    )
    parser.add_argument(
        "-a", "--apikey", help="Api key for the Rest API of CHN", type=str
    )
    ...
    try:
        ...
        local_ip = "127.0.0.1"
    parser.add_argument("-i", "--ip-address", help="IP of honeypot", default=local_ip)
    parser.add_argument("-u", "--url", help="CHN Server to register to", required=True)
    parser.add_argument(
        "-k", "--no-verify", help="Do not verify TLS connection", action="store_true"
    )
    parser.add_argument("-o", "--state-output", help="State output file", type=str)
...
def main():
    ...
    if os.path.exists(args.state_output):
        ...
        try:
            ...
            logging.error("Could not decode state file in to json, overwriting it")
            logging.error("%s" % e)
            overwrite = True
            existing_data = None
        ...
        if existing_data:
            if "honeypot" not in existing_data:
                logging.error("State file does not include honeypot, overwriting")
                overwrite = True
    else:
        overwrite = True
