...
from mhn.api.models import Sensor, DeployScript as Script
...
from mhn.common.utils import paginate_options, alchemy_pages, mongo_pages
...
def get_sensors():
    ...
    return render_template(
        "ui/sensors.html", sensors=sensors, view="ui.get_sensors", pag=pag
    )
...
def forgot_passwd(hashstr):
    ...
    return render_template("ui/reset-password.html", reset_user=user, hashstr=hashstr)
...
def get_credentials_payloads(clio):
    ...
    credentials_payloads += clio.hpfeed.get_payloads(
        {"limit": 10000}, {"channel": "kippo.sessions"}
    )[2]
    credentials_payloads += clio.hpfeed.get_payloads(
        {"limit": 10000}, {"channel": "cowrie.sessions"}
    )[2]
...
def graph_passwords():
    ...
    bar_chart = pygal.Bar(
        style=LightColorizedStyle, show_x_labels=True, config=PYGAL_CONFIG
    )
...
def top_kippo_cowrie_attackers(clio):
    ...
    return [
        ...
        for ip, count in sorted(grouped.items(), key=lambda x: x[1], reverse=True)
    ]
